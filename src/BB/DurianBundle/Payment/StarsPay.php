 ...
 * 星和易通
 */
class StarsPay extends PaymentBase
{
    /**
     * 支付提交參數
     *
    ...
    public function getVerifyData()
    {
        ...
        if (in_array($this->options['paymentVendorId'], [1090, 1092, 1103, 1107, 1111])) {
            ...
            // 判斷返回是否為html格式，與技術確認若商號被停用回傳為html格式，其餘狀況都是xml格式
            $html = (0 !== strpos($result, '<?xml'));

            if ($html) {
                // 取支付平台回傳的訊息
                $crawler = new Crawler();
                $crawler->addHtmlContent($result);
                $nodeList = $crawler->filterXPath('//div[@class="alert-icon"]');
                ...
                // 取回傳的訊息
                $message = $nodeList->text();

                if ($message == '') {
                    throw new PaymentConnectionException('Get pay parameters failed', 180128, $this->getEntryId());
                }

                throw new PaymentConnectionException($message, 180130, $this->getEntryId());
            }
