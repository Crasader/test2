 ...
 *　星付支付
 */
class StarPay extends PaymentBase
{
    /**
     * 支付提交參數
     *
    ...
    public function getVerifyData()
    {
        ...
        if (in_array($this->options['paymentVendorId'], [1090, 1092, 1103, 1107, 1111])) {
            ...
            $parseData = $parseData['detail'];

            if (!isset($parseData['code']) || !isset($parseData['desc'])) {
                throw new PaymentConnectionException('Get pay parameters failed', 180128, $this->getEntryId());
            }

            if ($parseData['code'] !== '00') {
                throw new PaymentConnectionException($parseData['desc'], 180130, $this->getEntryId());
            }
            ...
            $this->setQrcode(base64_decode($parseData['qrCode']));
