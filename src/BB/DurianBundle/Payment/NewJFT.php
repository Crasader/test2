 ...
 * 3. 驗證結果是否相符
 */
class NewJFT extends PaymentBase
{
    /**
     * 支付時要傳給平台驗證的參數
     *
    ...
    public function verifyOrderPayment($entry)
    {
        ...
        //如果沒有返回新聚付通的訂單號要丟例外
        if (!isset($this->options['aorderid'])) {
            throw new PaymentException('No return parameter specified', 180137);
        }

        //確認是否為新聚付通的返回
        //url: http://do.jftpay.net/chargebank.aspx
        $curlParam = [
            'method' => 'GET',
            'uri' => '/chargebank.aspx',
            'ip' => $this->options['verify_ip'],
            'host' => $this->options['verify_url'],
            'param' => http_build_query(['jftorderid' => $this->options['aorderid']]),
            'header' => []
        ];
