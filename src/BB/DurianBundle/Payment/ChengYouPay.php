 ...
 * 誠優支付
 */
class ChengYouPay extends PaymentBase
{
    /**
     * 支付時要傳給平台驗證的參數
     *
    ...
    public function getVerifyData()
    {
        ...
        if (!isset($parseData['pay_code'])) {
            throw new PaymentConnectionException('Get pay parameters failed', 180128, $this->getEntryId());
        }

        if ($parseData['pay_code'] !== 'HL0000') {
            if (isset($parseData['pay_msg'])) {
                throw new PaymentConnectionException($parseData['pay_msg'], 180130, $this->getEntryId());
            }
