 ...
 * 中商通
 */
class JungShangTung extends PaymentBase
{
    /**
     * 支付時要傳給平台驗證的參數
     *
    ...
    public function getVerifyData()
    {
        ...
        $this->requestData['sign'] = $signData['sign'];
        $this->requestData['sign2'] = $signData['sign2'];
    ...
    protected function encode()
    {
        ...
        $encodeSign2Str = http_build_query($encodeData);

        // sign 加密串排除 money
        unset($encodeData['money']);
        $encodeSignStr = http_build_query($encodeData);

        $signData = [
            'sign' => strtolower(md5($encodeSignStr)),
            'sign2' => strtolower(md5($encodeSign2Str)),
        ];
