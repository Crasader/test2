 ...
 * 3. 驗證結果是否相符
 */
class ZhihPay extends PaymentBase
{
    /**
     * 支付時要傳給平台驗證的參數
     *
    ...
    protected $bankMap = [
        ...
        '220' => 'HZB', // 杭州銀行
        '222' => 'NBB', // 寧波銀行
        '1090' => 'weixin_scan', // 微信二維
        '1092' => 'alipay_scan', // 支付寶二維
        '1097' => 'weixin_h5api', // 微信_手機支付
        '1098' => 'alipay_h5api', // 支付寶_手機支付
        '1103' => 'tenpay_scan', // QQ二維
        '1104' => 'qq_h5api', // QQ_手機支付
    ];
    ...
    public function getVerifyData()
    {
        ...
        if (in_array($this->options['paymentVendorId'], [1090, 1092, 1097, 1098, 1103, 1104])) {
            ...
            $result = $this->verifyToGetPostUrl();
            ...
            if (!isset($parseData['qrcode']) && !isset($parseData['payURL'])) {
                throw new PaymentConnectionException('Get pay parameters failed', 180128, $this->getEntryId());
            }

            // 手機支付
            if (in_array($this->options['paymentVendorId'], [1097, 1098, 1104])) {
                return $this->getPhonePayPostUrlAndData($parseData['payURL']);
            }
        ...
        // 調整網銀提交網址
        $postUrl = 'https://pay.' . $this->options['postUrl'] . '/gateway?input_charset=' . $this->requestData['input_charset'];
     ...
     */
    private function verifyToGetPostUrl()
    {
        // 調整提交網址
     ...
     * @return array
     */
    private function getPhonePayPostUrlAndData($payUrl)
    {
        $return = [
            'post_url' => urldecode($payUrl),
            'params' => []
        ];

        // 微信手機支付需解析提交網址並使用GET才能正常跳轉
        if ($this->options['paymentVendorId'] == 1097) {
            $parseUrl = parse_url($return['post_url']);
            ...
            if (isset($parseUrl['query'])) {
                parse_str($parseUrl['query'], $return['params']);
            }
