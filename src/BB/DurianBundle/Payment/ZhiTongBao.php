 ...
 * 智通寶支付
 */
class ZhiTongBao extends PaymentBase
{
    /**
     * 支付時要傳給平台驗證的參數
     *
    ...
    public function getVerifyData()
    {
        ...
        $postUrl = sprintf(
            ...
            '/gateway?input_charset=',
            $this->requestData['input_charset']
        );

        // 網銀
        if (!in_array($this->options['paymentVendorId'], [1088, 1090, 1092, 1097, 1098, 1103, 1104, 1108])) {
            return $this->getOnlinePayAndH5Params($postUrl);
        }
        ...
        // H5手機支付
        if (in_array($this->options['paymentVendorId'], [1088, 1108])) {
            return $this->getOnlinePayAndH5Params($postUrl);
        }
        ...
        $this->checkPayReturnData($parseData);
     ...
     * @return array
     */
    private function getOnlinePayAndH5Params($postUrl)
    {
        // 設定支付平台需要的加密串
        $this->requestData['sign'] = $this->encode();
     ...
     */
    private function checkPayReturnData($parseData)
    {
