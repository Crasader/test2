 ...
 * 摩寶支付
 */
class MoBaoPay extends PaymentBase
{
    /**
     * 支付提交參數
     *
    ...
    protected $requestData = [
        'apiName' => 'WEB_PAY_B2C', // WAP_PAY_B2C: 手機支付，WEB_PAY_B2C: 網頁支付
    ...
    protected $bankMap = [
        ...
        16 => 'PSBC', // 中國郵政
        17 => 'BOC', // 中國銀行
        1090 => 'Mobaopay' // 微信支付,因沒有直連接口,目前作法是跳轉到摩寶收銀台再選擇掃碼支付
    ];
    ...
    public function verifyOrderPayment($entry)
    {
        ...
        $status = openssl_verify($encodeStr, $signMsg, $this->getRsaPublicKey(), OPENSSL_ALGO_MD5);
    ...
    public function paymentTracking()
    {
        ...
        // 驗證簽名
        if (!openssl_verify($parseData['data'], $parseData['sign'], $this->getRsaPublicKey(), OPENSSL_ALGO_MD5)) {
            throw new PaymentConnectionException('Signature verification failed', 180034, $this->getEntryId());
        }
    ...
    public function getPaymentTrackingData()
    {
        ...
        $curlParam = [
            ...
            'path' => '/cgi-bin/netpayment/pay_gate.cgi',
            'method' => 'POST',
    ...
    private function parseData($content)
    {
        ...
        preg_match('{<signMsg>(.*?)</signMsg>}', $content, $match);
        $parseData['sign'] = base64_decode($match[1]);
