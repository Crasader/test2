 ...
 * 沃支付
 */
class WoozfPay extends PaymentBase
{
    /**
     * 支付提交參數
     *
    ...
    public function getVerifyData()
    {
        ...
        // 判斷notify_url的host是否存在
        if (!isset($parseNotifyUrl['host'])) {
        ...
        $this->requestData['req_referer'] = $parseNotifyUrl['host'];
    ...
    public function paymentTrackingVerify()
    {
        ...
        if ($parseData['response']['is_success'] == 'FALSE' && isset($parseData['response']['error_msg'])) {
            throw new PaymentConnectionException($parseData['response']['error_msg'], 180130, $this->getEntryId());
        }
