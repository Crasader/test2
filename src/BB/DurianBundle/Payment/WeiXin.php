 ...
 * 微信支付
 */
class WeiXin extends PaymentBase
{
    /**
     * 生成預付單提交參數
     *
    ...
    public function getVerifyData()
    {
        ...
        $this->requestData['body'] = $extra['remark'];
        ...
        // remark為選填欄位，因此若無值則傳送「购物商品」文字
        if (!$this->requestData['body']) {
            $this->requestData['body'] = '购物商品';
        }

        // 二維支付product_id必填
        if ($this->options['paymentVendorId'] == 1090) {
            $this->requestData['trade_type'] = 'NATIVE';
            $this->requestData['product_id'] = $this->options['orderId'];
        }
    ...
    private function getPrepay()
    {
        ...
        $package = [
            ...
            'noncestr' => $parseData['nonce_str'],
            'package' => 'Sign=WXPay',
            'partnerid' => $parseData['mch_id'],
            'prepayid' => $parseData['prepay_id'],
            'timestamp' => time(),
        ];

        ksort($package);

        $package['key'] = $this->privateKey;
        $packageStr = urldecode(http_build_query($package));

        unset($package['key']);

        // 設定加密簽名
        $package['sign'] = strtoupper(md5($packageStr));

        // 二維支付時 code_url 為二維碼網址
        if ($parseData['trade_type'] == 'NATIVE') {
