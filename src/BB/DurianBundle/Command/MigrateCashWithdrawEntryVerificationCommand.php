 ...
 * 轉移出款紀錄明細資料驗證
 */
class MigrateCashWithdrawEntryVerificationCommand extends ContainerAwareCommand
{
    /**
     * @var OutputInterface
     */
    ...
    {
        $this
            ->setName('durian:migrate:cash-withdraw-entry-verification')
            ->setDescription('轉移出款紀錄明細資料驗證')
            ...
            ->setHelp(<<<EOT
轉移出款紀錄明細資料驗證, 驗證到小於等於指定ID的資料
app/console durian:migrate:cash-withdraw-entry-verification 80000000

平日轉移出款紀錄明細資料驗證, 驗證到小於等於指定ID的資料
app/console durian:migrate:cash-withdraw-entry-verification 80000000 --sleep --start_id=123
EOT
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln("Start Verify New Cash Withdraw Entry...");
        ...
        $sqlColumn = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = 'cash_withdraw_entry_old'";
        ...
        $sqlNextId = 'SELECT MAX(id) FROM (SELECT id FROM cash_withdraw_entry_old ' .
            'WHERE id > ? AND id <= ? ORDER BY id ASC LIMIT 10000) sub';

        $verifyQuery = "SELECT old.id FROM cash_withdraw_entry_old old " .
            "LEFT JOIN cash_withdraw_entry new ON (old.id = new.id and old.at = new.at) " .
        ...
        $output->writeln("Verify Cash Withdraw Entry Finish");
    }
