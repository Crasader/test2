 ...
 */
class SyncCashFakeNegativeCommand extends ContainerAwareCommand
{
    /**
     * @var OutputInterface
     */
    ...
    {
        $this
            ->setName('durian:sync-cash-fake-negative')
            ->setDescription('同步負數假現金')
            ->setHelp(<<<EOT
同步負數假現金
app/console durian:sync-cash-fake-negative
EOT
             );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $bgMonitor->commandStart('sync-cash-fake-negative');
    ...
    private function process()
    {
        ...
        try {
            while ($count++ < 1000) {
                $queue = $redis->rpop('cash_fake_negative_queue');
                ...
                $negEntity = $em->find('BBDurianBundle:CashFakeNegative', $params);
                ...
                if (!$negEntity) {
                    $negEntity = new CashFakeNegative($userId, $currency);
                    $negEntity->setCashFakeId($negMsg['cash_fake_id']);
                    $em->persist($negEntity);
                }

                $version = $negEntity->getVersion();

                $updated = !$version || $version < $negMsg['cash_fake_version'];

                if ($updated) {
                    $negEntity->setBalance($negMsg['balance']);
                    $negEntity->setVersion($negMsg['cash_fake_version']);
                }
            ...
            foreach ($queues as $q) {
                $redis->lpush('cash_fake_negative_queue', $q);
            }
        }
