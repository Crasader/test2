 ...
 * 更新層級會員人數
 */
class UpdateLevelCountCommand extends ContainerAwareCommand
{
    /**
     * @var OutputInterface
     */
    ...
    {
        $this->setName('durian:update-level-count')
            ->setDescription('更新層級會員人數')
            ->addOption('wait-time', null, InputOption::VALUE_OPTIONAL, '等待時間(單位毫秒)', null)
            ->setHelp(<<<EOT
更新 level user_count 欄位
app/console durian:update-level-count --wait-time=500000
EOT
             );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $bgMonitor->commandStart('update-level-count');
        ...
        try {
            while ($msgNum < 1000) {
                $queue = $redis->lpop('level_user_count_queue');
                $queueMsg = null;
            ...
            foreach ($colQueue as $levelId => $changeCount) {
                ...
                $msg = "[$now] levelId:$levelId, changeCount:$changeCount";
                $msgArray[] = $msg;
            }
            ...
            foreach ($colQueue as $levelId => $changeCount) {
                if ($changeCount) {
                    ...
                    $redis->lpush('level_user_count_queue', json_encode($data));
                }
            }
