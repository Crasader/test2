 ...
 * 檢查現金交易明細資料
 */
class CheckCashEntryCommand extends ContainerAwareCommand
{
    /**
     * @var InputInterface
     */
    ...
    {
        $this
            ->setName('durian:cronjob:check-cash-entry')
            ->setDescription('檢查CashEntry及history資料庫資料')
            ...
            ->setHelp(<<<EOT
檢查CashEntry及history資料庫資料
$ ./console durian:cronjob:check-cash-entry

指定要檢查的時間區間
$ ./console durian:cronjob:check-cash-entry --starttime="2012/1/1 12:00:00" --endtime="2012/1/2 12:00:00"
EOT
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $bgMonitor->commandStart('check-cash-entry');
        ...
        if ($this->update) {
            ...
            $this->log("Update CashEntry difference start at ".$startTime->format('Y-m-d H:i:s'));
            $msgNum = $this->updateCashEntry();
            $endTime = new \DateTime('now');
            $this->log("Update CashEntry difference finish at ".$endTime->format('Y-m-d H:i:s'));
        } else {
            $startTime = new \DateTime('now');
            $this->log("Check CashEntry difference start at ".$startTime->format('Y-m-d H:i:s'));
            $msgNum = $this->checkCashEntry();
            $endTime = new \DateTime('now');
            $this->log("Check CashEntry difference finish at ".$endTime->format('Y-m-d H:i:s'));
        }
    ...
    private function checkCashEntry()
    {
        ...
        do {
            ...
            $query = "SELECT id, created_at, at, cash_id, user_id, currency, opcode, amount, memo, balance, ref_id, cash_version ".
                     "FROM cash_entry ".
                     "WHERE at >= ? ".
                     "AND at < ? ".
                     "ORDER BY id;";
            $results = $entryConn->fetchAll($query, [$startTime, $endTime]);
            ...
            $hisQuery = "SELECT id, created_at, at, cash_id, user_id, currency, opcode, amount, memo, balance, ref_id, cash_version ".
                        "FROM cash_entry ".
                        "WHERE at >= ? ".
                        "AND at < ? ".
                        "ORDER BY id;";
            ...
            foreach ($results as $result) {
                ...
                $existQuery = "SELECT id FROM cash_entry WHERE id = ? LIMIT 0, 1;";
                ...
                $msg = sprintf(
                    'CashEntry: id: %d, cashId: %d, userId: %d, currency: %d, opcode: %d, amount: %d, balance: %d, cash_version: %d',
                    $result['id'],
                    $result['cash_id'],
                    $result['user_id'],
                    $result['currency'],
        ...
        try {
            ...
            if ($insertResults) {
                ...
                $insertSql = $this->setInsertSql('cash_entry', $columns, $insertResults);
            ...
            if ($updateResults) {
                ...
                $insertSql = $this->setInsertSql('cash_entry_diff', $columns, $updateResults, true);
     ...
     * @throws \Exception
     */
    private function updateCashEntry()
    {
        $em = $this->getEntityManager();
        ...
        $diffQuery = "SELECT id FROM cash_entry_diff";
        ...
        $results = $entryConn->executeQuery($query, $params, $types)->fetchAll();

        // 只更新 created_at 及 at 欄位
        $columns = array('created_at', 'at');
        $sql = $this->setUpdateSql('cash_entry', $columns, $results);
        $executeCount = 0;
        ...
        if ($this->update && !$this->dryRun) {
            ...
            $deleteSql = $this->setDeleteSql('cash_entry_diff', $results);
    ...
    private function log($msg)
    {
        if (null === $this->logger) {
            $this->logger = $this->getContainer()->get('durian.logger_manager')
                ->setUpLogger('check_cash_entry.log');
        }
