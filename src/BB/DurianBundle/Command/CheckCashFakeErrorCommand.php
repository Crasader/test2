 ...
 * 更新快開額度不符名單
 */
class CheckCashFakeErrorCommand extends AbstractCheckErrorCommand
{
    /**
    ...
    {
        $this
            ->setName('durian:cronjob:check-cash-fake-error')
            ->setDescription('更新快開額度不符名單')
...
檢查目前時間的前1小時內資料
$ ./console durian:cronjob:check-cash-fake-error

檢查6/23 12:00:00 - 13:00:00 時間區間
$ ./console durian:cronjob:check-cash-fake-error --begin="2012/06/23 12:00:00" --end="2012/06/23 13:00:00"

檢查6/23 12:00:00 - 13:00:00 時間區間，並檢查最後一筆明細是否有遺失
$ ./console durian:cronjob:check-cash-fake-error --begin="2012/06/23 12:00:00" --end="2012/06/23 13:00:00" --check-last

執行但只顯示語法，不更新資料庫
$ ./console durian:cronjob:check-cash-fake-error --dry-run
EOT
            );
    }
    ...
    {
        return CashFake::NUMBER_OF_DECIMAL_PLACES;
    }
    ...
    protected function getEntryByTimeInterval($beginAt, $endAt)
    {
        ...
        $qb->select(
            "e.id, e.at, Concat(e.cashFakeId, '/', e.userId, '/', e.currency) as major_key," .
            "e.amount, e.balance , e.cashFakeVersion as version"
        );

        $qb->from('BBDurianBundle:CashFakeEntry', 'e')
            ->where('e.at >= :begin')
            ->andWhere('e.at < :end')
            ->orderBy('e.cashFakeVersion', 'ASC')
    ...
    protected function saveErrorEntry($majorKey, $entry)
    {
        ...
        list($cashFakeId, $userId, $currency) = explode('/', $majorKey);
        $totalAmount = $entry['amount'];
        $balance = $entry['final_balance'];

        $item = new CashFakeError();
        $item->setCashFakeId($cashFakeId);
    ...
    {
        list($cashFakeId, $userId) = explode('/', $majorKey);
        $totalAmount = $entry['amount'];
        $balance = $entry['final_balance'];

        $msg = sprintf(
            'CashFakeError: cashFakeId: %d, userId: %d, balance: %.4f, amount: %.4f',
            $cashFakeId,
            $userId,
            $balance,
            $totalAmount
    ...
    protected function getPrevEntry($majorKey, $id, $version, $at, $history = false)
    {
        ...
        $keys = explode('/', $majorKey);
        $userId = $keys[1];
        $qb = $this->getEntityManager($emName)->createQueryBuilder();
        $qb->select(
            'e.id, e.at, e.cashFakeId as cash_fake_id, e.userId as user_id, e.currency, ' .
            'e.amount, e.balance, e.cashFakeVersion as version'
        );
        $qb->from('BBDurianBundle:CashFakeEntry', 'e')
    ...
    protected function getHasEntryData($beginAt, $endAt, $startId)
    {
        ...
        $qb->from('BBDurianBundle:CashFake', 'c')
        ...
        $cashFake = $qb->getQuery()->getArrayResult();
    ...
    protected function getLastEntryByTime($targetTime, $userId)
    {
        ...
        $qb->select('e.balance, e.cashFakeVersion as version');
        $qb->from('BBDurianBundle:CashFakeEntry', 'e')
            ->where('e.userId = :userId')
            ->andWhere('e.at >= :at')
            ->setParameter('userId', $userId)
    ...
    {
        return "cash_fake_balance_{$userId}_{$currency}";
    }
}
